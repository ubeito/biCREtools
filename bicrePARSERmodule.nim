...
type Tokenkind* = enum avachar, initpos, accpos, chclass, escchar, dotany #!!!
...
#    var i0: int # starting position index of token 
    while (true): # :>1
      helperAr = (true, @[], @[]) #!!
      inc(cA) #!!
      if ptt[i] != '|' and ptt[i] != '$' and ptt[i] != ')': # for branch to <:12
        ...
        while (true): ## :>2
          ...
          if ptt[i] == '(': ### :>3 
            ...
            helperPr = (false,@[i],@[i]) #!!
            if ptt[i] == '[': i += ptt[i..^1].find(']'); token[i0] = (chclass, ptt[i0+1..<i]) # i0 = i;  # skip '[...]' for char class
...
when isMainModule: 
  ...
  # r":>(0<:|0)": bugfixed # r":>[abc]<:" # r":>\|<:" # r":>a|a<:" # r":>a*<:" # r":>a+<:" # r":>a|<:" #:non-accept
...
#     | +----------- a OR [X] OR \b ---------->◎-- * OR + -->◎   <== <:32, <:35
#     |\                                     ↑／|\            ↑    ↑: node labeled ':>' code, ↑: node labeled '?' code
#     | +------------ :> OR :>^b ----------->◎ | +---- ? --->◎   <== <:33, <:36
#      \                                     ↑／                   ↑: node labeled '<:' code
#       +------------ <: OR <:_b ----------->◎                    <== <:34
# [※ If rule "F -> ε" is added, its look-ahead chars are {(,a,...}.] ==> instead rule E -> ε added
